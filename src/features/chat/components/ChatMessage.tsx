...
export default function ChatMessage({ message, onAction }: ChatMessageProps) {
  ...
  return (
    <div className={`flex items-start space-x-3${!isIA ? ' justify-end' : ''}`}>
      {isIA && (
        <div className="flex-shrink-0 w-8 h-8 bg-purple-900/50 rounded-full flex items-center justify-center border border-purple-700/50">
      ...
      {!isIA && (
        <div className="flex-shrink-0 w-8 h-8 bg-pink-900/50 rounded-full flex items-center justify-center border border-pink-700/50">
